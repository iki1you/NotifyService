...
namespace Adapters.GreenAPI.Services
{
    ...
    {
        private const string MockApiUrlEnvironmentVariable = "GREENAPI_MOCK_URL";
        ...
        public async Task<OperationResult<GreenApiSendResponse>> SendFileByUrlAsync(FileMessage message, long credentialId)
        {
            ...
            var apiUrl = ResolveApiUrl(credential.ApiUrl);
            var url = $"{apiUrl}/waInstance{credential.IdInstance}/sendFileByUrl/{credential.ApiTokenInstance}";

            var sw = Stopwatch.StartNew();
            var sendResult = await httpClient.SendPostRequestAsync(url, message);
            sw.Stop();

            GreenApiMetrics.RecordExternalCallDuration(sw.Elapsed.TotalMilliseconds, "send_file_by_url", sendResult.IsSuccess ? "success" : "failed");

            if (sendResult.IsFail)
            {
                GreenApiMetrics.IncrementExternalCallFailed("send_file_by_url");
        ...
        public async Task<OperationResult<GreenApiSendResponse>> SendMessageAsync(TextMessage message, long credentialId)
        {
            ...
            var apiUrl = ResolveApiUrl(credential.ApiUrl);
            var url = $"{apiUrl}/waInstance{credential.IdInstance}/sendMessage/{credential.ApiTokenInstance}";

            var sw = Stopwatch.StartNew();
            var sendResult = await httpClient.SendPostRequestAsync(url, message);
            sw.Stop();

            GreenApiMetrics.RecordExternalCallDuration(sw.Elapsed.TotalMilliseconds, "send_message", sendResult.IsSuccess ? "success" : "failed");

            if (sendResult.IsFail)
            {
                GreenApiMetrics.IncrementExternalCallFailed("send_message");
                return sendResult.Error;
            }
            try
            {
        ...
        private static string ResolveApiUrl(string sourceApiUrl)
        {
            ...
            {
                return sourceApiUrl.TrimEnd('/');
            }

            var mockApiUrl = Environment.GetEnvironmentVariable(MockApiUrlEnvironmentVariable);
            if (string.IsNullOrWhiteSpace(mockApiUrl))
            {
                return sourceApiUrl.TrimEnd('/');
            }

            return mockApiUrl.TrimEnd('/');
        }
    }
}
